 ...
 * (đóng dấu 1 sự cố lên Sổ cái minh bạch — hành động thủ công, vĩnh viễn).
 */

import { NextRequest, NextResponse } from 'next/server';
import { SensorEvent } from '@/lib/consensus';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`[API/blockchain] ⛓ Đã đóng dấu sự kiện ${event_id} → TxHash: ${tx_hash.slice(0, 16)}...`);
